...
Hermes Chat / TUI / Desktop
  └─ MCP tools: orchestra_*
       └─ pinodes-orchestra backend (:3847)
            ...
            └─ PtyHub → pi | hermes | claude | codex nodes
...
- Current runtimes are `pi`, `hermes`, `claude`, and `codex`.
...
| Agent nodes | **Keep existing runtimes** | `pi`, `hermes`, `claude`, and `codex` are shipped. PTY runtimes share the text sentinel contract; Codex uses the structured runtime path. |
